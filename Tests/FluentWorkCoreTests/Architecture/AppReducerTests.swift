...
/// The settings screen is the kill switch now that T4 put `voiceProcessing`
/// in `firstWave`. An override that can be set but not cleared would leave a
/// device stuck off (or on) with nothing on it able to undo that, short of
/// reinstalling.
///
/// Note what is *not* asserted: that the override survives a relaunch. It does
/// not — `localOverrides` lives in memory — and that is the point.
@Test func voiceProcessingCanBeTurnedOffByOverrideAndClearedBackToTheDefault() throws {
    let store = TestStore(
    ...
    store.send(.featureFlags(.setLocalOverride(flag: .voiceProcessing, isEnabled: false)))
    #expect(!store.state.featureFlags.isEnabled(.voiceProcessing))
    #expect(store.state.featureFlags.localOverrides[.voiceProcessing] == false)
