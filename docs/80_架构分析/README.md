...
### 第四处，性质不同：`03` §4.4 的**前提**是错的（2026-09-22，`70_/22_`）

前三处是**计数错**，这一处是**立论错**，所以单独说。

`03` §4.4 把 `pcmBuffer` / `isCapturingSpeech` 记成一条「隐式不变量」：安全性靠 `removeAll()` 的调用顺序，而没有任何测试会红；`05` S1-1 据此把它列为「本清单里最像定时炸弹的一条」，并进了「如果只做三件事」的第 2 条。

**查证结果：`pcmBuffer` 零读点。** 改动前 9 个出现位置 = 2 个声明 + 6 个写 + **1 个读**，而那唯一的读（`if isCapturingSpeech {`）只是决定要不要往这个死 buffer 里写。**整条数据流是闭环，闭环外没有观察者。** 所以「调整顺序会串上一场的音频」这个后果**不可能发生**——串音频需要有人读到它。

处置是**删除**（`70_/22_`，D12）。原文的两条建议都不可行：收进 `SpeechCaptureBuffer` 类型等于给「泵持有每会话状态」发一张通行证；「不清空必须红」的测试**写不出来**（无可观察后果，且断言私有局部量等于把死代码编码进测试）。

**这一处的教训与前二处不同**，值得记下：前三处是「数字数错了，事实本身没错」；这一处是**「描述完全准确，只是描述的对象是死的」**。§4.4 说的每一句都成立——有跨会话的 buffer、有清空、清空在 `beginSpeech` 之前、没有测试会红。**唯独没问一句「谁在读它」。** 前提未验证时，分析越细致，错得越像对的。同族的还有 `70_/13_`（D5 推翻自身立论）与 `70_/16_`/`17_`（判据从不取真值）——**先验证前提，再决定动作。**

### 第五处，又回到计数错：S1-9 的「8 个」实际是 11 条（2026-09-23，`70_/24_`）

`05` S1-9 原文列了 8 个采集诊断 mark 名，并据此说「没有任何测试钉住它们」。写 `70_/24_` 时逐条枚举 `audioEventPump` 里的 `timings.mark`，实际是 **11 条**——漏掉的 `vad_speech_start`、`audio_uplink_turn`、`audio_engine_failed` **不属于 `.capture*` 命名族**，按前缀扫就会漏。

**这与上表前三处是同一个错法**，只是漏法不同：`OSAllocatedUnfairLock` 是「命中里混进了注释」（多算），`waitUntil` 是「多行调用被漏掉」（少算），这一处是「按前缀扫一族，漏掉不共享前缀的成员」（少算）。**三次的教训可以合并成一句：数完数字，再问一句「命中的是不是都算数，没命中的是不是都不算数」。**

已在 `05` 更正为 11 条，并由 `70_/24_` 全部钉住（生产代码零改动）。
