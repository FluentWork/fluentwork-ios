...
> - 「`audioEngine.play(frame:)` 是实际出声的唯一路径」（§1、§2、§4）—— 该入口已在 2026-09-20 随引擎侧水印一起**删除**（legacy 路径随 `d004869` 失效，函数本体随后删除，理由见 `07` §2）；唯一的出声路径是 `TTSPlaybackCoordinator` → `AudioSink.play(pcm:)` → 引擎。
>
...
| `BargeInAudioGate` | `Networking/Socket/` | 传输层第一道水印 | **已删除**（2026-09-22，`18_`） |
