...
> **修订（2026-09-22，`18_`）：这一节的结论已被推翻——「该保留的是另一道」是错的。** 传输层那道**也删了**，理由与本文件删掉引擎侧那道**逐字相同**：序号说不出一个帧属于哪一轮。
>
> 本节表格为它辩护的六条（覆盖面、有真实生产者、有释放点、有上报、有真传输测试）**每一条都成立**，但它们回答的是「它接得上吗」，而不是「它拦得住吗」。答案和引擎侧那道一样：**拦不住它被声称要拦的东西**——WS 保序，所以打断**之后**到达的帧序号必然**大于**水位，一律放行。而它**恰好是两道里唯一能无声吞掉整轮音频的**：网关编号每轮从 0 起，水位武装期间到达的下一轮 `0..N` 全部 ≤ 水位。
>
> 保留原文，因为它是「当时为什么认为传输层那道不一样」的完整记录。结论以 [`18_`](./18_删除传输层序号水印.md) 为准；下表标题里的「（保留）」应读作「（当时保留，2026-09-22 已删）」。

| | 引擎侧 `AudioPlaybackGate`（已删） | 传输层 `BargeInAudioGate`（当时保留，2026-09-22 已删） |
|---|---|---|
...
> **修订（2026-09-22）**：上面这两句的**前半句已不成立**——现在没有任何一层按序号丢弃音频。**后半句仍然成立，而且被加强了**：归属**只**由 `TTSPlaybackCoordinator` 回答，因为另一条按序号猜的路已经不存在了。见 [`18_`](./18_删除传输层序号水印.md)。
