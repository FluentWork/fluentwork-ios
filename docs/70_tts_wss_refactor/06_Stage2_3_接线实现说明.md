...
`AudioEngine` 自己当 sink 去播。**今天的行为与接线前逐字节相同**，因为网关还没发
`ai.tts.start`；网关一旦开始发，音频不会再被认领进一台录音机 —— 这正是 2026-09-12 的静音。
...
结果是两条路径各有一道门：legacy 帧归水位线，keyed 帧归轮次注册表。契约要求 legacy
回滚能力长期存在（网关停发 start 即退回 fallback），所以水位线**不能删**。
...
- **今天的行为**：零变化。网关不发 `ai.tts.start`，所有帧走 legacy，`play(legacy:)` 默认实现
  就是 `play(frame:)`。
- **网关打开 start 之后**：帧按轮归属，解码走 `WSAudioFrameDecoderAdapter`（复用
  `wsAudioFrameDecoder` 这个工厂 —— 一个 codec 实现，两个调用点）。
- **观测**：新增 `tts_frame_dropped`（reason∈{superseded, unknownTurn}）埋点。丢弃从此有日志；
  `tts_decoder_failed` 保留，语义收窄为「解码失败」。
- **未完成**：① 真机 smoke（出声、打断不串音、首响）—— 按 `03` 的风险表，这是接线后
  必须做的那一次；② 网关开始发 `ai.tts.start`（另一仓，按契约 §2 在①之后）；
  ③ Stage 4 删死路径。
