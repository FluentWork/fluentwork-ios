...
`18_` 与 `19_` 是这一轮里仅有的两条改动**生产行为**的（其余五条是补效果、改判据、或改测试）。`18_` 也是唯一一条**删掉一整道防线**的改动，所以论证写得最细：为什么「补水位生命周期」不够、为什么收益是数量级不对称的、以及 `77_` P1-22 的可观测性关切迁到了哪里。
...
**仍未关闭**：`11_` §7 的三条待决问题（D5 失败策略、Stage 3 的 `turn_id` 上帧、两个设计分支的处置）；以及 D6/D7/D8/D9/D11 五条未修缺陷。真机验证（`08_` §3）仍未做。
