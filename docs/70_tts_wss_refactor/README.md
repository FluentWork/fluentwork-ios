...
**仍未关闭**：`11_` §7 的三条待决问题（D5 失败策略、Stage 3 的 `turn_id` 上帧、两个设计分支的处置）；**累计待决**——**D13**（客户端 ASR 家族 279 行零引用，是放弃还是保留，`22_` §9）、**D9**（中间件 1735 行的**切缝选择**，推荐改法已更正为不成立，`23_` §10）、**3 处 `catch` 的 `return nil`**（同 D14 形状，需先定「每会话报一次」的语义，`23_` §7）、**`24_` §7(b)(c)**（是否把「埋点名」的钉法扩到 `speech_turn_ended` 与 `transportEventPump` 一侧——建议做，但另立票）。以及 D7/D8 两条未修缺陷（D11 由 `20_` 关闭、D6 由 `21_` 关闭、D12 由 `22_` 关闭、**D14 由 `23_` 关闭**、**S1-9 由 `24_` 关闭**）。真机验证（`08_` §3）仍未做。
