...
| 水印在传输 + 引擎两处 | `gate` 一处，且按轮重置 —— **未采用**：协调器里从未建过 `PlaybackWatermark`（全仓零命中）。实际收到一处的是反过来的那一半：引擎那道 `AudioPlaybackGate` 被删了（它不可能被武装，见 `07` §2），barge-in 的丢弃只剩传输层 `BargeInAudioGate`。方向不同：不是"把两处合成一处放进协调器"，而是"删掉多余的那一处" |
