...
> **修订（2026-09-20）**：这段过渡期策略被**反向执行**了。实际落地的兼容分支是「`turn_id == nil` → **丢弃**」，不是「→ 走旧路径」：`TTSPlaybackCoordinator.onAudio` 对无归属的帧返回 `.dropped(reason: .unknownTurn)`（`:154-156`）。`audioEngine.play(frame:)` 还在代码里，但已无生产调用点。「两侧解耦、可独立回滚」也没有兑现——网关开始发 `ai.tts.start` 与客户端删掉 legacy 回退发生在同一个窗口，任一侧单独倒退的产物都是静音，不是降级。
...
> **修订（2026-09-20）**：这个搬移也没发生。`EngineAudioSink` 写出来过，但从未接线，已在 Stage 4 删除。落地选的是**「引擎自己就是 sink」**（`AudioEngineProtocol: AudioSink`，`AppDependencies.swift:74-80`），理由是 `play(frame:)` 上的 `AudioPlaybackGate` 与 `playbackRetired` 两个守卫不能被绕过——换一个对象去播，症状分别是「打断后接着响」和「结束练习后又被迟到帧拉起来」（`06` §2.1）。所以下面这条「引擎不再直接对下行帧做决策」只对了一半：引擎确实不再做**轮次归属**的决策，但它的**水位线守卫原地未动**。
...
> **修订（2026-09-20）**：这一节描述的重构**没有发生**。`TransportEventRouter.swift` 写出来了、也有自己的测试（`Tests/FluentWorkCoreTests/Architecture/TransportEventRouterTests.swift`），但**没有任何生产调用点**——中间件根本没过它。`SpeechSessionMiddleware.swift` 现在是 1470 行（本节原写 1495 行），`transportEventPump` 仍是一个大 switch，路由、计时、埋点、错误四件事照旧挤在里面。Stage 1 实际交付的是一个**未被接线的组件**，这是本系列里唯一一处至今没有关闭的缺口。

目标里多出的一个薄组件：它只做**路由**，不做业务。`transportEventPump` 现在一个 case 干四件事，拆成「路由器把帧投给正确的 owner」：
...
flowchart LR
    ...
    R -->|audio| TC[TTSPlaybackCoordinator]
    R -->|control: tts| TC
    R -->|control: session| SM[SpeechSessionMachine]
    R -->|control: badge| BD[BadgeSink]
    R -->|control: error| ER[ErrorHandler]
    R -->|telemetry| TL[Telemetry\nfirstResponse/timings]
```

原方案的预期是：中间件的 `transportEventPump` 从 1495 行收敛为「一张路由表 + 各 owner 的调用」，计时与埋点各自回到自己的 owner（`Telemetry`），不再挤在一个 switch 里。**这个预期没有兑现**：中间件没有被拆，计时与埋点仍在那个 switch 里；`TransportEventRouter` 至今只有测试在调它。
