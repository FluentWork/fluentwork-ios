...
> **补充（2026-09-20 当天第二次）**：本文 §3.2 当时画的那两条路由器测试，在本轮接线时落地了——但**不是**照着当时的草稿写的。实际落在
> `Tests/FluentWorkCoreTests/Architecture/TransportRoutingEquivalenceTests.swift`，共三条：
> `routerDispatchesEveryControlFrameToExactlyOneOwner`（19 类样本各归一个 owner，抓手工镜像的写错/写重）、
> `socketFailureIsRoutedRatherThanDropped`（**对旧实现是红的**——旧的 router 把 `.failure` 硬编码成忽略；实测过）、
> 以及生产路由表的 handler 级测试（`.feedbackBadge` 必须 mark 评测等待、`.error` 仍触发 teardown、11 类未注册帧零 dispatch）。
>
> **修订（2026-09-22，`20_`）**：上面那句原写「13 类」。`.aiAudioChunk` 在 `20_` 里拿到了专属 handler（只上报、不播），
> 于是未注册的是 12 类，其中 `.error` 有动作、11 类零 dispatch。那份名单现在**逐条带理由**，并有一条集合相等断言——
> 新增一类 `WSControlFrame` 必须被分类，否则红。原文保留，改动只此一句。
>
> 还补了一条本文没预见到的形状：**用生产工厂（`makeTransportEventRouter`）驱动**，而不是测试自己拼一张表——
> 后者只能证明「路由器会分发」，证明不了「接线接对了」。原计划里那条"生产路由表键集合"的断言最终没写：
> 实测把 `.aiTTSStart` 从表里摘掉会让 7 条端到端测试变红，覆盖已经存在，再加一个只读内部表的公开 API 是重复的。
