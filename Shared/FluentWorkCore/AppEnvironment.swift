...
public struct AppEnvironment: Equatable, Sendable {
    ...
    public static let local = AppEnvironment(
        kind: .local,
        apiBaseURL: URL(string: "http://192.168.2.111:8080/api/v1")!,
        wssBaseURL: URL(string: "ws://192.168.2.111:8081/v1/voice")!,
        minimumLogLevelIsDebug: true
    )

    /// Create a local environment with custom host IP
    /// - Parameter host: The local machine's IP address (e.g., "192.168.2.104")
